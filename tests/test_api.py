...
def test_extract_brief_success():
    ...
    with open(pdf_path, "rb") as f:
        response = client.post(
            "/extract-brief", files={"file": ("brief_sample.pdf", f, "application/pdf")}
        )
...
def test_extract_brief_failure():
    ...
    response = client.post(
        "/extract-brief", files={"file": ("empty.txt", b"", "text/plain")}
    )
...
def test_extract_brief_internal_error():
    ...
    with pytest.MonkeyPatch.context() as monkeypatch:
        ...
        response = client.post(
            "/extract-brief",
            files={"file": ("brief_sample.pdf", b"dummy content", "application/pdf")},
        )
